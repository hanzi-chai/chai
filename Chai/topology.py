...
def quadSolve(a, b, c):
    ...
    if delta > 0:
        ...
        x1 = (-b + s)/2/a
        x2 = (-b - s)/2/a
        if -TOL < x1 < 1 + TOL and -TOL < x2 < 1 + TOL:
...
if __name__ == "__main__":
    ...
    TOPO = {topology(Char(nameChar, [Stroke(stroke) for stroke in strokeList])) for nameChar, strokeList in WEN.items()}

    with open('文.topo.yaml', encoding='utf-8', mode='w') as f:
