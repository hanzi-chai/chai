...
class Pictorial(CharacterFeatureExtractor):
    '''字根拆分器'''
    # TODO: 此处 __init__ 函数仍需要分离代码，与 config 相关的东西应该写在 configParser 模块。
    def __init__(self,
        ...
        charaterNames: List[str] = []
        for character in characters:
            charaterNames.append(character.name)
            if isinstance(character, Component):
                self.components[character.name] = character
            elif isinstance(character, Compound):
                self.compounds[character.name] = character
            else:
                raise TypeError("Invalid character")
        cpnDeps = loadComponentsDependencies(charaterNames)
        for cpn in cpnDeps:
            self.components[cpn.name] = cpn
        cpdDeps = loadCompoundsDependencies(charaterNames)
        for cpd in cpdDeps:
            self.compounds[cpd.name] = cpd
        self.componentRoots: Dict[str, Component] = {}
        self.compoundRoots: Dict[str, None] = {}
    ...
    def generateComponentScheme(self):
        '''
        找出部件在根集中的所有可行组合。
        ...
        for component in self.components.values():
            ...
            schemeBinary = self.select(component)
            component.scheme = tuple(component.binaryDict[x] for x in schemeBinary)

    def generateCompoundScheme(self):
        for compound in self.compounds.values():
            compound.scheme = compound.firstChild.scheme + compound.secondChild.scheme

    def extract(self):
        self.generateComponentScheme()
        self.generateCompoundScheme()
