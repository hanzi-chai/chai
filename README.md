...
为应对字拆分的变异性，「拆」采用了比较激进的方案——枚举切片寻找拆分可能性。即先对一个字含有的所有笔画，每种取笔组合都尝试一下，看看该组合是否存在于用户定义字根当中，获得一个有效切片集合，然后看看这些切片当中，哪些组合起来可以形成我们的字。  

我们先说切片。对于一个字的切片，如何表达我要在字里面取哪些笔？一种比较自然的思路是布尔向量：  
- 假设字 $C$ 是含有 $5$ 笔的字，它的笔画状态的布尔向量应该是 $(1, 1, 1, 1, 1)$
- 这时候我要取字 $C$ 前 $2$ 笔作为一个切片 $S$ ，那么 $S$ 对于 $C$ 的切片布尔向量应该是 $(1, 1, 0, 0, 0)$
- 取完切片 $S$ 之后，余下部分 $R$ 对于 $C$ 的剩余布尔向量应该是 $(0, 0, 1, 1, 1)$
  

于是我们很自然的就联想到是不是能使用二进制数来表达字、字根、切片，并且通过位运算来快速处理切片算法。  
答案是可以：
- 一个含有 $n$ 笔的字可用一个十进制数 $2^n-1$ 表达其笔画状态，例如一个 $5$ 笔的字可以用 $31$ 来表示，其对应二进值是 $11111$
- 对于字的任意切片，可同理表示，上字含首末笔的切片为 $10001$，对应十进值为 $17$ ，取完之后的剩余是 $1110$ ，对应的十进值为 $14$ ,可以看到，$17+14=31$
  

我们通过遍历$[1,2^n-1]$内的所有数字来寻找一个字的所有有效切片，基于位运算：
```python
def genPowerDict(objectChar, degenerator, degeneracy):
    objectChar.powerDict = {} # 将幂集返回给对象字
    l = objectChar.charlen # 捕获对象字的笔画数
    mask = [1 << (l-i-1) for i in range(l)] # 生成掩码，二进制分别为 1，10，100……
    for k in range(1, 2**l):  # 遍历[1,2^n)——幂集因而得名，切片数量由关于n的幂函数决定
        sliceStrokeList = []
        for idx, item in enumerate(mask): # 对于每个数（切片），遍历所有掩码
            if k & item:  # 数 k 与某个掩码按位与后，如果不为 0，那么说明某一位为 1
                sliceStrokeList.append(objectChar.strokeList[idx]) # 此时添加这一位对应的笔画
        # 内部循环完成了将形如 110001 的切片，转换成含有相应笔画的笔画序列（此例中是首二末三个笔）
        characteristicString = degenerator(Char('', sliceStrokeList)) # 退化这个切片
        objectChar.powerDict[k] = degeneracy.get(characteristicString)
        # 在退化字典中索引用户字，并将代表切片的数k及对应的用户字根作为一个键值对存储在幂集字典中
        # 没有对应用户字根的切片，其值将会记为None
```

下面，计算拆分的算法也基于位运算

### 理解拆分
「拆」的拆分逻辑很简单，当我们有了一个基于用户字根退化值的索引字典之后：
- 在字上找到一个切片
- 退化这个切片，看看这个切片是不是用户字根，如果是，加入到拆分列
- 寻找下一个切片
  

「拆」对每次切片有一个限定的原则，就是这次切片，必须包含被切对象的第一笔，于是完整的流程是：
- 设「当前字」是第一个「待拆对象」
- 获取所有包含第一笔的切片，我们假设有$N$个
- 经过比对，这$N$个切片里面实际有$n$个是用户字根，那么我们同时生成$n$个拆分，拆分的结构是$(S, R)$元组
- 设每一个「剩余」是「待拆对象」，重复上述操作，直到「剩余」跟其「切片」相等，即拆完
- 注：每次拆分分支都生成多个完整的拆分元组$(S1, S2, ..., Sn, R)$

```python
def nextRoot(n):
    '''
    给出当前切片的下一个可能切片列表，可能切片应该包含输入切片的第一笔
    '''
    powerList = [0]
    while n: # 直到当前序列所有「1」位都被置0之前，做：
        t = n & (n-1) # 找到右边第一个「1」，如1110010的右二位，将其置0，得余待检序列1110000
        m = n - t # 当前序列扣除余待检序列，获得当前位及其右边所有位，1110010-1110000=10
        n = t # 将余待检序列设为当前序列，用于下一loop
        cp = [x + m for x in powerList] # 对列表中每一个已有位扩增当前位「1」
        powerList += cp # 以当前位列表扩增原列表
        # 当前位的「0」选项，将会在下一位「1」扩增时扩增
    return powerList[len(powerList)//2:] # 将所有不足笔数长度的序列剔除，表明所取切片必含输入切片的第一笔
...
择优函数通过序贯处理筛查准则，输出一个单一的拆分——不符合筛查准则的都被筛去。  
筛查准则为一个拆分进行评分，通过评分，我们可以找到拆分集里的极值，极值对应的拆分就是筛查准则下最优拆分。  
**以下伪代码**：
```python
def sieve(scheme): # 某个筛查准则
    evaluation = do something with scheme
    return evaluation # 返回一个评估值

def select(sieve, schemeList): # 利用某个筛查准则筛选拆分集
    evalList = [sieve(scheme) for scheme in schemeList] # 对拆分集里面每个拆分进行评估
    bestEval = min(evalList) # 找到最优评估值
    selectedSchemeList = [scheme for scheme, evaluation in zip(schemeList, evalList)
                          if evaluation == bestEval] # 筛选
    return selectedSchemeList # 将筛选过的列表返回

def selector(schemeList):
    for sieve in sieves: # 对于定义好的每个筛选准则，序贯处理
        schemeList = select(sieve, schemeList)
    if len(schemeList) == 1:
        bestScheme = do something with schemeList[0]
    else:
        raise Error(告知用户所设筛查准则并不能获得单一拆分)
    return bestScheme
```
...
狭义上包括了同一性判据、切片方式、择优函数等部分的逻辑序列或其混合形式。  

然而，可能存在跨嵌套结构的切片，并且从广义上来看，管理嵌套结构也应该是拆分的一部分。  
